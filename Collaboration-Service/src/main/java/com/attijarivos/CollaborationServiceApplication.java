...
public class CollaborationServiceApplication {
	...
	CommandLineRunner start(CollaborationRepository collaborationRepository) {
		return args -> {
			...
			collaborationRepository.save(
					Collaboration.builder().titre("Collaboration 3").dateCreationCollaboration(new Date()).dateDepart(new Date()).confidentielle(false).IdProprietaire(idProprietaire).visible(true).build()
			);
